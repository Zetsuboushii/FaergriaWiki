# Elementar

## Detonaroc {collapsible="true" default-state="expanded"}

<img src="detonaroc.png" alt="" height="400"/>

| HP | AC | Move        |
|----|----|-------------|
| 85 | 15 | Roll 30 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| +4  | -1  | +3  | -5  | +0  | -3  |

| Typ       | Weakness  | Resistances | Immunities                          |
|-----------|-----------|-------------|-------------------------------------|
| Elementar | Psychisch |             | Gift, Psychisch, Betäubt, Erschöpft |

| Ability          | Description                                                                                                                                                     |
|------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Rollangriff      | Bonusaktion zur Fortbewegung, kann Angriffe von Gelegenheit provozieren.                                                                                        |
| Selbstzerstörung | Explodiert bei Kontakt in Nahkampfreichweite, löst einen DexSave DC 15 aus -> 6d6 Feuerschaden im Radius von 10 ft., halbierter Schaden bei erfolgreichem Save. |

| Action | Description        | Attack    | Damage            |
|--------|--------------------|-----------|-------------------|
| Rammen | Melee, Reach 5 ft. | +6 to hit | 2d8+4 bludgeoning |

## Eisflügler {collapsible="true" default-state="expanded"}

<img src="eisfluegler.png" alt="" height="400"/>

| HP | AC | Move               |
|----|----|--------------------|
| 16 | 14 | 10 ft., fly 60 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| -3  | +3  | +1  | -4  | +1  | -2  |

| Typ    | Weakness | Resistances | Immunities      |
|--------|----------|-------------|-----------------|
| Bestie | Feuer    |             | Cold, Paralysed |

| Ability       | Description                                             |
|---------------|---------------------------------------------------------|
| Frostige Aura | Verursacht bei feindlichem physischen Angriff 1d8 cold. |
| Dunkelsicht   | Kann im Dunkeln sehen, ohne Lichtquellen zu benötigen.  |

| Action            | Description                                                          | Attack          | Damage                                                       |
|-------------------|----------------------------------------------------------------------|-----------------|--------------------------------------------------------------|
| Eisbiss           | Melee, Reach 5 ft., ein Ziel                                         | +5 to hit       | 1d4+3 piercing plus 1d4 cold                                 |
| Frostige Schwinge | Erzeugt eine Schockwelle aus frostiger Luft, die Gegner verlangsamt. | Dexterity DC 13 | 1d6 cold und 10 ft. reduziert bis zum Ende des nächsten Zugs |

## Eisgeist {collapsible="true" default-state="expanded"}

<img src="eisgeist.png" alt="" height="400"/>

| HP | AC | Move               |
|----|----|--------------------|
| 85 | 17 | 30 ft., fly 40 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| +2  | +4  | +2  | +1  | +3  | +2  |

| Typ   | Weakness | Resistances                                                 | Immunities               |
|-------|----------|-------------------------------------------------------------|--------------------------|
| Untot | Feuer    | Bludgeoning, Piercing, and Slashing from Nonmagical Attacks | Kälte, Poison, Erschöpft |

| Ability              | Description                                                         |
|----------------------|---------------------------------------------------------------------|
| Frostige Präsenz     | Kälteschaden für Kreaturen, die sich innerhalb von 10 Fuß befinden. |
| Geisterhafte Rüstung | Kann physische Angriffe teilweise ignorieren.                       |

| Action           | Description                               | Attack    | Damage                             |
|------------------|-------------------------------------------|-----------|------------------------------------|
| Kälteklinge      | Melee, Reach 10 ft., ein Ziel             | +7 to hit | 2d8+4 slashing plus 2d6 cold       |
| Frostschrei      | 15 ft. cone, alle Kreaturen in Reichweite | DC 15 Con | 3d8 cold und Betäubung für 1 Runde |
| Eisiger Ausbruch | Ranged, 30 ft., ein Ziel                  | +6 to hit | 3d6 cold                           |

## Flammentänzerin {collapsible="true" default-state="expanded"}

<img src="flammentaenzerin.png" alt="" height="400"/>

| HP  | AC | Move   |
|-----|----|--------|
| 102 | 15 | 50 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| +3  | +4  | +2  | +1  | +3  | +5  |

| Typ       | Weakness        | Resistances  | Immunities             |
|-----------|-----------------|--------------|------------------------|
| Elementar | Physischer Kern | Nichtmagisch | Feuer, Betäubung, Gift |

| Ability      | Description                                                                                          |
|--------------|------------------------------------------------------------------------------------------------------|
| Feuergleiten | Kann sich geschmeidig bewegen und hinterlässt eine Spur aus Flammen, die Schaden verursacht.         |
| Kern         | If the core is destroyed, the creature explodes, dealing 6d6 fire damage to everything within 20 ft. |

| Action         | Description         | Attack    | Damage             | Additional Effect                                                                                   |
|----------------|---------------------|-----------|--------------------|-----------------------------------------------------------------------------------------------------|
| Flammenklingen | Melee, Reach 10 ft. | +8 to hit | 2d10+4 fire damage | Target catches on fire, taking 2d6 fire damage at the start of each of its turns until extinguished |

## Frostspeer {collapsible="true" default-state="expanded"}

<img src="frostspeer.png" alt="" height="400"/>

| HP | AC | Move   |
|----|----|--------|
| 40 | 13 | 20 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| +3  | +2  | +0  | -2  | +1  | -3  |

| Typ       | Weakness                            | Resistances | Immunities      |
|-----------|-------------------------------------|-------------|-----------------|
| Elementar | Bludgeoning (zweimal => insta kill) | Kälte       | Gift, Erschöpft |

| Ability         | Description                                                                                  |
|-----------------|----------------------------------------------------------------------------------------------|
| Spröde Struktur | Zerstört nach zwei erfolgreichen Bludgeoning-Angriffen aufgrund der zerbrechlichen Struktur. |

| Action           | Description                               | Attack    | Damage                                                                      |
|------------------|-------------------------------------------|-----------|-----------------------------------------------------------------------------|
| Eisspeerwurf     | Ranged, 30/60 ft., ein Ziel               | +6 to hit | 2d8+3 piercing plus 2d6 cold                                                |
| Eisiger Windstoß | 15 ft. cone, alle Kreaturen in Reichweite | DC 13 Con | 2d6 cold und Disadvantage auf Angriffswürfe bis zum Ende des nächsten Zuges |

## Racheschnee {collapsible="true" default-state="expanded"}

<img src="racheschnee.png" alt="" height="400"/>

| HP | AC | Move   |
|----|----|--------|
| 50 | 12 | 20 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| +3  | -1  | +3  | -4  | +0  | -3  |

| Typ       | Weakness | Resistances | Immunities      |
|-----------|----------|-------------|-----------------|
| Elementar | Feuer    | Kälte       | Gift, Erschöpft |

| Ability            | Description                                                                   |
|--------------------|-------------------------------------------------------------------------------|
| Eisdurchdringung   | Kann sich durch Eis und Schnee bewegen, ohne verlangsamt zu werden.           |
| Gefrorene Barriere | Kann seine AC für eine Runde um 2 erhöhen, indem er eine Schneeschicht formt. |

| Action              | Description                                                                 | Attack    | Damage                                           |
|---------------------|-----------------------------------------------------------------------------|-----------|--------------------------------------------------|
| Schneekugelwurf     | Ranged, 30/60 ft., ein Ziel                                                 | +5 to hit | 2d6+3 bludgeoning und 1d6 cold                   |
| Frostiger Aufprall  | Melee, Reach 5 ft., ein Ziel                                                | +5 to hit | 1d8+3 bludgeoning                                |
| Schneesturm (1/Tag) | 20 ft. radius, verursacht Kälteschaden und schwieriges Gelände für 1 Minute | DC 14 Con | 3d6 cold und Geschwindigkeit um 10 ft. reduziert |

## Schneewolf {collapsible="true" default-state="expanded"}

<img src="schneewolf.png" alt="" height="400"/>

| HP | AC | Move   |
|----|----|--------|
| 45 | 14 | 40 ft. |

| Str | Dex | Con | Int | Wis | Cha |
|-----|-----|-----|-----|-----|-----|
| +3  | +2  | +3  | -4  | +2  | -1  |

| Typ    | Weakness | Resistances | Immunities    |
|--------|----------|-------------|---------------|
| Bestie | Feuer    | Kälte       | Betäubt, Gift |

| Ability              | Description                                                                            |
|----------------------|----------------------------------------------------------------------------------------|
| Rudel Taktik         | Erhält Vorteil auf Angriffe, wenn ein Verbündeter innerhalb von 5 ft. des Ziels steht. |
| Eisiger Atem         | Kann einen frostigen Atemstoß ausführen, der Gegner in einem Kegel trifft.             |
| Geisterhafte Präsenz | Reduziert die Sicht in einem Umkreis von 10 ft. durch einen frostigen Nebel.           |

| Action       | Description                                         | Attack         | Damage                                                                           |
|--------------|-----------------------------------------------------|----------------|----------------------------------------------------------------------------------|
| Biss         | Melee, Reach 5 ft., ein Ziel                        | +5 to hit      | 2d6+3 piercing plus 1d6 cold                                                     |
| Eisiger Atem | 15 ft. cone, alle Kreaturen in Reichweite           | DC 13 Con      | 3d6 cold und Geschwindigkeit um 10 ft. reduziert bis zum Ende des nächsten Zuges |
| Heulen       | Verursacht Furcht bei Gegnern im Umkreis von 10 ft. | DC 12 Wis Save | -                                                                                |

## Zunderflügler {collapsible="true" default-state="expanded"}

<img src="zunderfluegler.png" alt="" height="400"/>

| HP | AC | Move               |
|----|----|--------------------|
| 22 | 14 | 10 ft., fly 60 ft. |

| Str     | Dex    | Con    | Int     | Wis    | Cha     |
|---------|--------|--------|---------|--------|---------|
| -1 (-5) | 4 (+4) | 1 (+1) | -4 (-5) | 0 (+0) | -1 (-2) |

| Typ       | Weakness | Resistances                                             | Immunities      |
|-----------|----------|---------------------------------------------------------|-----------------|
| Elementar | Wasser   | Bludgeoning, Piercing, Slashing from Nonmagical Attacks | Fire, Exhausted |

| Ability         | Description                                                                        |
|-----------------|------------------------------------------------------------------------------------|
| Flammenkörper   | Move through fire without damage; contact deals 1d4 fire damage                    |
| Licht der Asche | Emits bright light in 10-ft radius when hit, potentially blinding nearby creatures |

| Action    | Description        | Attack    | Damage                    |
|-----------|--------------------|-----------|---------------------------|
| Feuerbiss | Melee, Reach 5 ft. | +4 to hit | 1d4+4 piercing + 1d6 fire |